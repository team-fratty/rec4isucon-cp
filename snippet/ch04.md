...
* リスト 4.59 N+1 問題が起きる模擬コード
```go
// DBアクセス回数：N回
for _, userID := range userIDs {
    ...
    sqlx.Get(q, &user, "SELECT name FROM `users` WHERE `id` = ?", userID)
    ...
}
```

* リスト 4.60 N+1 問題を解消した模擬コード
```go
// DBアクセス回数：1回
sqlx.In("SELECT name FROM `users` WHERE `id` IN (?)", userIds)
...
```

* リスト 4.61 計測区間 getTransactions:part3 にあるN+1問題の形
```go
...
